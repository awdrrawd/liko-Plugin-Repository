...
(async () => {
    ...
    assert.equal(history[0].expiresAt - history[0].translatedAt, 1800000);
    ...
    assert.equal((await api.translateReceivedText('hello', { priority: 'manual' })).error, 'disabled');
    assert.equal((await api.translateSentText('hello', { priority: 'manual' })).error, 'disabled');
    ctx.config.translateReceived = false;
    ctx.config.translateSent = false;
    assert.equal((await api.translate('manual while disabled', 'ja')).error, 'disabled');
    const beforeManual = calls;
    const manual = await api.translate('manual while disabled', 'ja', { priority: 'manual' });
    assert.equal(manual.translated, 'ja:manual while disabled');
    assert.equal(calls, beforeManual + 1);
    assert.equal(api.getCachedTranslation('manual while disabled', 'ja').translated, manual.translated);
    await api.translate('manual while disabled', 'ja', { priority: 'manual' });
    assert.equal(calls, beforeManual + 1, 'manual translation shares the MAT cache');
    assert.equal(ctx.config.enabled, false);
    assert.equal((await api.translateReceivedText('hello')).error, 'disabled');
    assert.equal((await api.translateSentText('hello')).error, 'disabled');
