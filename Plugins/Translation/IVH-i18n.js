...
(function () {
    ...
    const IVH_STRINGS = {
        ...
        'remoteEditSent': { TW: '📤 已送出對 {name} 的文本編輯', CN: '📤 已送出对 {name} 的文本编辑', EN: '📤 Sent text edits to {name}', DE: '📤 Textänderungen an {name} gesendet', FR: '📤 Modifications envoyées à {name}', RU: '📤 Правки текста отправлены {name}', UA: '📤 Правки тексту надіслано {name}' },
        ...
        'whitelistD': { TW: '會員編號，逗號或空白分隔。各類「白名單」編輯權限共用此名單。', CN: '会员编号，逗号或空白分隔。各类「白名单」编辑权限共用此名单。', EN: 'Member numbers, comma/space separated. Shared by every "Whitelist" edit permission.', DE: 'Mitgliedsnummern, durch Komma/Leerzeichen getrennt. Von allen „Whitelist"-Bearbeitungsrechten geteilt.', FR: 'Numéros de membre, séparés par virgule/espace. Partagé par toutes les permissions « Liste blanche ».', RU: 'Номера участников через запятую/пробел. Общий для всех прав «Белый список».', UA: 'Номери учасників через кому/пробіл. Спільний для всіх прав «Білий список».' },
        ...
        // ── 文本設定 ───────────────────────────────────────────────────
        'textsHint': { TW: '每行一句。$me＝被催眠者名稱、$n＝換行（彈幕／人影）；狀態訊息以 $a 開頭會以 Action 發送。', CN: '每行一句。$me＝被催眠者名称、$n＝换行（弹幕／人影）；状态信息以 $a 开头会以 Action 发送。', EN: 'One line per sentence. $me = hypnotized person, $n = line break (danmaku/ghost); status lines starting with $a are sent as Action.', DE: 'Eine Zeile pro Satz. $me = hypnotisierte Person, $n = Zeilenumbruch (Danmaku/Schemen); Statuszeilen mit $a werden als Aktion gesendet.', FR: 'Une ligne par phrase. $me = personne hypnotisée, $n = saut de ligne (danmaku/ombre) ; les lignes de statut commençant par $a sont envoyées en Action.', RU: 'Одна строка — одна фраза. $me = загипнотизированный, $n = перенос строки (данмаку/тень); строки статуса с $a отправляются как Action.', UA: 'Один рядок — одна фраза. $me = загіпнотизований, $n = перенос рядка (данмаку/тінь); рядки статусу з $a надсилаються як Action.' },
        'hypnoTextD': { TW: '彈幕／人影旁白來源，會和 BCX 的聽我聲音一起使用，僅被催眠者能看見。可用 $n 換行。', CN: '弹幕／人影旁白来源，会和 BCX 的听我声音一起使用，仅被催眠者能看见。可用 $n 换行。', EN: 'Source for danmaku/ghost narration; used together with BCX "listen to me", only the hypnotized sees it. Use $n for a line break.', DE: 'Quelle für Danmaku/Schemen-Erzählung; zusammen mit BCX „hör mir zu", nur der Hypnotisierte sieht es. $n für Zeilenumbruch.', FR: 'Source de la narration danmaku/ombre ; utilisée avec « écoute-moi » de BCX, visible par l\'hypnotisé seul. $n pour un saut de ligne.', RU: 'Источник текста данмаку/тени; вместе с BCX «слушай меня», видит только загипнотизированный. $n — перенос строки.', UA: 'Джерело тексту данмаку/тіні; разом із BCX «слухай мене», бачить лише загіпнотизований. $n — перенос рядка.' },
        ...
        'statusMsgD': { TW: '觸發催眠時隨機發送的訊息。以 $a 開頭＝Action（系統動作訊息），否則為 Emote。', CN: '触发催眠时随机发送的信息。以 $a 开头＝Action（系统动作信息），否则为 Emote。', EN: 'Message sent at random when hypnosis triggers. Starts with $a = Action (system message), otherwise Emote.', DE: 'Nachricht, die beim Auslösen zufällig gesendet wird. Beginnt mit $a = Aktion (Systemnachricht), sonst Emote.', FR: 'Message envoyé au hasard au déclenchement. Commence par $a = Action (message système), sinon Emote.', RU: 'Сообщение, отправляемое случайно при срабатывании. Начинается с $a = Action (системное), иначе Emote.', UA: 'Повідомлення, що надсилається випадково при спрацюванні. Починається з $a = Action (системне), інакше Emote.' },
        'statusMsgPh': { TW: '例：$a $me 的思緒變得混亂了', CN: '例：$a $me 的思绪变得混乱了', EN: 'e.g. $a $me\'s mind grows hazy', DE: 'z. B. $a $me wird benommen', FR: 'ex. $a $me a l\'esprit embrumé', RU: 'напр. $a $me разум туманится', UA: 'напр. $a $me розум туманіє' },
