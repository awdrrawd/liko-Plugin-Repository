...
(function () {
    ...
    const _strings = {
        ...
        'pcmLoadedCount':    { TW: 'PCM - {count}個載入成功', CN: 'PCM - {count}个加载成功', DE: 'PCM - {count} erfolgreich geladen', FR: 'PCM - {count} chargés avec succès', RU: 'PCM - успешно загружено {count}', UA: 'PCM - успішно завантажено {count}' },
        ...
        'pcmFailedCount':    { TW: 'PCM - {count}個載入失敗', CN: 'PCM - {count}个加载失败', DE: 'PCM - {count} konnten nicht geladen werden', FR: 'PCM - échec du chargement de {count}', RU: 'PCM - не удалось загрузить {count}', UA: 'PCM - не вдалося завантажити {count}' },
        'fusamFailedCount':  { TW: 'FUSAM - {count}個載入失敗', CN: 'FUSAM - {count}个加载失败', DE: 'FUSAM - {count} konnten nicht geladen werden', FR: 'FUSAM - échec du chargement de {count}', RU: 'FUSAM - не удалось загрузить {count}', UA: 'FUSAM - не вдалося завантажити {count}' },
        ...
        'customFieldType':   { TW: '載入方式（進階，不確定時請保留預設值）', CN: '加载方式（高级，不确定时请保留默认值）', DE: 'Lademethode (erweitert; im Zweifel Standard beibehalten)', FR: 'Méthode de chargement (avancé, conserver la valeur par défaut en cas de doute)', RU: 'Способ загрузки (для опытных; если не уверены, оставьте значение по умолчанию)', UA: 'Спосіб завантаження (для досвідчених; якщо не впевнені, залиште типове значення)' },
        'customTypeEval':    { TW: 'Eval — 將程式碼作為文字取得並執行（預設）', CN: 'Eval — 将代码作为文本获取并运行（默认）', DE: 'Eval — Code als Text abrufen und ausführen (Standard)', FR: 'Eval — récupérer le code comme texte et l’exécuter (par défaut)', RU: 'Eval — получить код как текст и выполнить (по умолчанию)', UA: 'Eval — отримати код як текст і виконати (типово)' },
        'customTypeScr':     { TW: 'Script — 使用 <script src>；適用於主機以 CORS 阻擋 fetch() 時', CN: 'Script — 使用 <script src>；适用于主机通过 CORS 阻止 fetch() 时', DE: 'Script — <script src>; verwenden, wenn der Host fetch() durch CORS blockiert', FR: 'Script — <script src>, à utiliser si l’hôte bloque fetch() avec CORS', RU: 'Script — <script src>; используйте, если хост блокирует fetch() политикой CORS', UA: 'Script — <script src>; використовуйте, якщо хост блокує fetch() політикою CORS' },
        'customTypeMod':     { TW: 'Module — 使用 dynamic import()；適用於 Vite／Rollup ESM 套件', CN: 'Module — 使用 dynamic import()；适用于 Vite／Rollup ESM 包', DE: 'Module — dynamic import(), für Vite-/Rollup-ESM-Bundles', FR: 'Module — dynamic import(), pour les bundles ESM Vite/Rollup', RU: 'Module — dynamic import(), для ESM-сборок Vite/Rollup', UA: 'Module — dynamic import(), для ESM-збірок Vite/Rollup' },
