...
(function () {
  ...
  function isFilteredMessage(text, isPrivate) {
    return text.includes("BCX commands tutorial") || text.includes("BCX also provides") || text.includes("(输入 /help 查看命令列表)") || (isPrivate && (text.includes("悄悄話") || text.includes("好友私聊") || text.includes("BEEP")));
  }
  ...
  function getLabelColor(msg, nameButton) {
    ...
    // 嘗試從多個位置獲取顏色信息
    let c = msg.style?.getPropertyValue("--label-color") || getComputedStyle(msg).getPropertyValue("--label-color") || (nameButton && (nameButton.style?.getPropertyValue("--label-color") || getComputedStyle(nameButton).getPropertyValue("--label-color"))) || "";
    c = c.trim();
    if (c) return c;
  ...
  function showCustomPrompt(message, options = []) {
    return new Promise(function (resolve) {
      ...
      if (options.length === 0) {
        ...
        // 自定義選項按鈕
        buttons = options.map((opt, idx) => `<button data-value="${opt.value}" style="margin: 5px; padding: 8px 16px; cursor: pointer; background: #0066cc; color: #fff; border: none; border-radius: 4px;">${opt.text}</button>`).join("");
      }
  ...
  async function exportChatAsHTML(NoLeave = true, includePrivate = null) {
    ...
    `;
      ...
      for (const msg of messages) {
        ...
        } else if (msg.classList.contains("ChatMessageAction") || msg.classList.contains("ChatMessageActivity") || msg.classList.contains("ChatMessageEmote") || msg.classList.contains("ChatMessageEnterLeave")) {
          // 動作/活動/表情/進出消息
    ...
    if (NoLeave && includePrivate === null) {
      showCustomPrompt("請問您是否保存包含悄悄話(whisper)與私信(beep)的信息?")
        .then(processExport)
        .catch(function (e) {
  ...
  async function exportExcel() {
    ...
    try {
      ...
      messages.forEach((msg) => {
        ...
        // 格式化時間顯示
        const timeDisplay = typeof msg.time === "string" && msg.time.includes("T") ? new Date(msg.time).toLocaleString() : msg.time;
