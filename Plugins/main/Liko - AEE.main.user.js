...
`;
  ...
  <button class="layer-btn ${state.selectedLayer===idx?'selected':''}" data-select-layer="${idx}">
    ...
    // ── AppearancePreviewCleanup 防禦性 patch ─────────────────────────────────
    // BC + BCX 互動 bug：Character 陣列中某些 preview 角色的 CharacterID 為
    // undefined，導致 .startsWith() 爆炸。
    try {
        modApi.hookFunction("AppearancePreviewCleanup", 0, (args, next) => {
            try {
                ...
                try {
                    window.Character
                        .filter(c => c?.CharacterID?.startsWith?.("AppearancePreview-"))
                        .forEach(c => { try { CharacterDelete(c); } catch(e3) {} });
                } catch(e4) {}
            }
        });
    } catch(e) {}
    ...
    try {
        modApi.hookFunction("ItemColorFireExit", 0, (args, next) => {
            ...
            // 先存好，next() 之後這些全域變數可能已被清空
            let result;
            try {
                result = next(args);
            } catch(e) {
                // BC 原生或 LSCG 在 chain 中出錯，記錄但不 re-throw
                console.warn('[AEE] ItemColorFireExit chain error (suppressed):', e?.message ?? e);
                result = undefined;
            } finally {
